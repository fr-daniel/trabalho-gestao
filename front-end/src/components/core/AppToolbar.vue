<template>
  <v-toolbar app color="primary" dark fixed>
    <router-link :to="{ path: '/'}">
      <img alt="Logo NPI" src="/static/logo-01.png" />
    </router-link>
    <v-spacer></v-spacer>

    <v-toolbar-items class="ml-3" v-if="isAdministrador">
      <v-btn flat to="/coordenador/exames">Exames</v-btn>
      <v-btn flat to="/coordenador/usuarios">Usuários</v-btn>
    </v-toolbar-items>
    <v-toolbar-items class="ml-3" v-if="isFuncionario">
...
export default {
  ...
  computed: {
    ...mapGetters({
      isAdministrador: "auth/isAdministrador",
      isFuncionario: "auth/isFuncionario"
    })
  },
